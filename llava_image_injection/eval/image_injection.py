...
def load_param(MODEL_NAME, model, tokenizer, initial_query):
    ...
    return tokenizer, image_processor, vision_tower, unnorm, embeds, projector, prompt, input_ids
...
def train_image_entire(input_ids, X, y, model, vision_tower, projector, epochs=100, lr=0.01):
    ...
    for i in pbar:
        ...
        for j in range(y.shape[1]):
            ...
            cur_new_input_embeds = torch.cat((cur_input_embeds[:image_start_token_pos+1], 
                                              cur_image_features, 
                                              cur_input_embeds[image_start_token_pos + 256 + 1:]), dim=0)
...
def train_image_partial(input_ids, X, y, model, vision_tower, projector, epochs=100, lr=0.01, rows=10):
...
def save_image(X, unnorm, name='test'):
    # save image to .jpg
    save_img_path = '../../result_images/' + name + '.jpg'
    
    # save the image tensor to .pt
    save_pt_path = '../../result_images/' + name + '.pt'
